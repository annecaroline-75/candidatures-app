...
//    SUPABASE_KEY  -> cle secrete (sb_secret_...) ou service_role
//
//  Le nom exact importe peu : si ces noms ne sont pas trouves,
//  le fichier reconnait les variables d'apres leur contenu.
// =========================================================

// Une adresse Supabase valide
function estUrlSupabase(v) {
  return typeof v === "string" &&
    /^https:\/\/[a-z0-9-]+\.supabase\.(co|in)\/?$/i.test(v.trim());
}

// Une cle Supabase utilisable cote serveur
function estCleServeur(v) {
  if (typeof v !== "string") return false;
  const t = v.trim();
  if (t.startsWith("sb_secret_")) return true;
  if (t.split(".").length === 3) {
    try {
      const charge = JSON.parse(Buffer.from(t.split(".")[1], "base64").toString("utf8"));
...
function lireConfig() {
  ...
  // 1. Les noms habituels, testes en premier
  let url =
  ...
  // 2. Sinon, on reconnait les variables d'apres leur contenu,
  //    quel que soit le nom que tu leur as donne.
  if (!estUrlSupabase(url)) {
    const trouve = Object.keys(env).find(function (n) { return estUrlSupabase(env[n]); });
    if (trouve) url = env[trouve];
  }
  if (!cle) {
    const trouve = Object.keys(env).find(function (n) { return estCleServeur(env[n]); });
    if (trouve) cle = env[trouve];
  }

  return { url: (url || "").trim().replace(/\/+$/, ""), cle: (cle || "").trim() };
}

// Liste les noms de variables liees a Supabase, SANS leur valeur
function nomsVariablesSupabase() {
  return Object.keys(process.env).filter(function (n) {
    return /supa/i.test(n) || estUrlSupabase(process.env[n]) || estCleServeur(process.env[n]);
  });
}
...
export default async function handler(req, res) {
  ...
  if (!url || !cle) {
    const noms = nomsVariablesSupabase();
    return res.status(500).json({
      erreur: "Configuration Supabase introuvable cote serveur.",
      adresse_trouvee: url ? "oui" : "NON",
      cle_trouvee: cle ? "oui" : "NON",
      variables_vues_par_le_serveur: noms.length ? noms : "aucune variable liee a Supabase",
      a_faire: noms.length
        ? "Ces variables existent mais leur contenu ne correspond pas. Verifie l'adresse (https://xxx.supabase.co) et la cle (sb_secret_... ou service_role)."
        : "Le serveur ne voit aucune variable. Verifie qu'elles sont cochees pour l'environnement Production sur Vercel, puis relance un deploiement."
    });
  }
